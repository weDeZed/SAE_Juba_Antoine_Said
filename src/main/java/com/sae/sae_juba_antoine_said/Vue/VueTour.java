...
public class VueTour {
    ...
    public VueTour(Pane pane, Tour tours) {
        this.pane = pane;
        this.tours = tours;

        FileInputStream fichierTour = null;

        if (tours instanceof Tour){
            try {
        ...
        imageView.setLayoutX(tours.getX() - decalageL);
        imageView.setLayoutY(tours.getY() - decalageH);
        this.pane.getChildren().add(imageView);
        Circle rangeCircle = new Circle(tours.getX(), tours.getY(), tours.getRange()*16);
